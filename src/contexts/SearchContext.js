  ...
  switch (action.type) {
    ...
    case "SELECT_ROOM":
      ...
      if (state.bookedRooms.length >= state.searchedRooms.length) {
        console.log("Maximum rooms already selected");
        return state; // Return current state without making any changes
      }

      // Check if the selected room is already fully booked
      const selectedRoomInAvailableRooms = state.availableRooms.find((room) => room.id === selectedRoom.id);
      if (selectedRoomInAvailableRooms && selectedRoomInAvailableRooms.bookedRoomCount >= selectedRoomInAvailableRooms.totalRoomCount) {
        console.log("Room is fully booked");
        return state;
      }

      const updatedBookedRoomsS = [...state.bookedRooms, selectedRoom];

      const updatedAvailableRooms = state.availableRooms.map((room) => {
      ...
      return {
        ...state,
        bookedRooms: updatedBookedRoomsS,
        availableRooms: updatedAvailableRooms,
      };
    case "SELECTROOM_ADD":
      const roomToAddId = action.payload;
      const selectedRoomCount = state.bookedRooms.length;

      if (selectedRoomCount >= state.searchedRooms.length) {
        console.log("Maximum rooms already selected");
        return state; // Return current state without making any changes
      }

      const roomToAdd = state.availableRooms.find((room) => room.id === roomToAddId);

      if (roomToAdd && roomToAdd.bookedRoomCount >= roomToAdd.totalRoomCount) {
        console.log("Room is fully booked");
        return state; // Return current state without making any changes
      }

      // Check if the room is already booked
      if (state.bookedRooms.includes(roomToAddId)) {
        console.log("Room is already booked");
        return state; // Return current state without making any changes
      }

      const updatedAvailableRoomsAdd = state.availableRooms.map((room) => {
        if (room.id === roomToAddId.id) {
          const updatedRoom = {
            ...room,
      ...
      const updatedBookedRoomsADD = [...state.bookedRooms, roomToAddId];
      return {
        ...state,
        bookedRooms: updatedBookedRoomsADD,
        availableRooms: updatedAvailableRoomsAdd,
      };

    case "SELECTROOM_SUB":
      const roomToSubId = action.payload;
      console.log("Room to subtract:", roomToSubId);
      console.log("Booked rooms:", state.bookedRooms);
      const roomIndexToRemove = state.bookedRooms.findIndex((id) => id === roomToSubId.id);
      console.log(roomIndexToRemove);
      if (roomIndexToRemove === -1) {
        console.log("Room not found in booked rooms");
        return state; // Return current state without making any changes
      }
      const roomToSubtract = state.availableRooms.find((room) => room.id === roomToSubId);
      const updatedAvailableRoomsSub = state.availableRooms.map((room) => {
        if (room.id === roomToSubId.id) {
          const updatedRoom = {
            ...room,
            bookedRoomCount: room.bookedRoomCount > 0 ? room.bookedRoomCount - 1 : 0,
            isSelected: room.bookedRoomCount > 1 ? true : false,
          };
          return updatedRoom;
        }
        return room;
      });
      const updatedBookedRooms = [...state.bookedRooms.slice(0, roomIndexToRemove), ...state.bookedRooms.slice(roomIndexToRemove + 1)];
      console.log("Updated booked rooms:", updatedBookedRooms);
      return {
        ...state,
        bookedRooms: updatedBookedRooms,
        availableRooms: updatedAvailableRoomsSub,
      };
