...
export default function RoomListView({ room, index }) {
  ...
  const handleRoomAdd = (roomId, roomMaxOccupancy, bookedRoomCount) => {
    dispatch({ type: "SELECTROOM_ADD", payload: { id: roomId, maxOccupancy: roomMaxOccupancy, bookedRoomCount } });
  };

  const handleRoomSub = (roomId, roomMaxOccupancy, bookedRoomCount) => {
    dispatch({ type: "SELECTROOM_SUB", payload: { id: roomId, maxOccupancy: roomMaxOccupancy, bookedRoomCount } });
  };
  ...
  return (
    <div className="card" key={index}>
      <div className="row">
        <div className="price-container d-sm-block d-none">
        ...
        <div className="col-xl-8 col-lg-7 col-md-8 col-12">
          <div className="card-body">
            ...
            <div className="price-container d-sm-none d-block">
              {room.price} <small>/ night</small>
            </div>
          </div>
          <div className="card-footer">
            ...
            {room.isSelected || room.bookedRoomCount > 0 ? (
              <div className="room_counter">
                {/* {console.log(bookedRooms)}
                                {console.log(searchedRooms.length)} */}
                <p>Rooms</p>
                <Button variant="outlined" onClick={() => handleRoomSub(room.id, room.maxOccupancy, room.bookedRoomCount)}>
                  <RemoveOutlinedIcon />
                </Button>
                <span>{room.bookedRoomCount}</span>
                <Button variant="outlined" disabled={bookedRooms.length == searchedRooms.length} onClick={() => handleRoomAdd(room.id, room.maxOccupancy, room.bookedRoomCount)}>
                  <AddOutlinedIcon />
                </Button>
              </div>
            ) : (
